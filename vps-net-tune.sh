...
_record_systemd_state() {
  ...
  printf "%s\t%s\t%d\n" "$RPSXPS_UNIT_NAME" "$state" "$existed" >>"$SYSTEMD_STATE"
...
if [[ -f "$SYSTEMD_STATE" ]]; then
  if command -v systemctl >/dev/null 2>&1; then
    while IFS=\$'\\t' read -r unit state existed; do
      [[ -n "\$unit" ]] || continue
      if [[ "\$existed" == "1" ]]; then
        case "\$state" in
          enabled)
            systemctl enable --now "\$unit" >/dev/null 2>&1 || warn "failed to enable unit: \$unit"
            ;;
          masked)
            systemctl mask "\$unit" >/dev/null 2>&1 || warn "failed to mask unit: \$unit"
            ;;
          *)
            systemctl disable --now "\$unit" >/dev/null 2>&1 || warn "failed to disable unit: \$unit"
            ;;
        esac
      else
